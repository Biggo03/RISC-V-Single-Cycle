...
The design has a testbench for each individual component, as well as a top-level testbench. Some modules require the generation of test vectors through the use of the python script test_vector_generation.py. The top level testbench runs simple RISC-V assembly programs that produce a specific result that is checked for correctness. The programs to run have both their assembly and machine code versions within the 'Test Vector Generation'/riscvprograms folder. To run a given program, the file pointed to within the instruction memory must be changed.
